...
public class EcommerceAuthController : ControllerBase
{
    ...
    public async Task<ActionResult> ResetPassword([FromBody] ResetPasswordDto dto)
    {
        try
        {
            ...
            var customer = await _context.EcommerceCustomers
                .FirstOrDefaultAsync(c => c.PasswordResetToken == dto.Token);
    ...
    private string GenerateJwtToken(EcommerceCustomer customer)
    {
        ...
        var token = new JwtSecurityToken(
            issuer: _configuration["Jwt:Issuer"] ?? "NobleStepApi",
            audience: _configuration["Jwt:Audience"] ?? "NobleStepClient",
            claims: claims,
